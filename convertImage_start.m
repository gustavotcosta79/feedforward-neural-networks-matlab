...
baseFolder = 'C:\Users\gugav\OneDrive\Ambiente de Trabalho\ISEC\2ano\CR\TPCR2425\TPCR2425\start';
...
for classIdx = 1:numClasses % percorrer cada classe (numero de pastas)
    ...
    for j = 1:imgsPerClass
        ...
        % Converter para escala de cinzentos (se a imagem for a cores)
        if size(img, 3) == 3
            img = rgb2gray(img);
        end

        % Redimensionar e binarizar
        img = imresize(img, imageSize); %50 por 50
        img = imbinarize(img); % converte para imagem binaria
